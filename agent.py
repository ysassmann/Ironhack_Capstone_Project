    ...
    ** Citation section:**
    ...
    def _create_tools(self) -> List[Tool]:
        ...
        def find_failure_cases(query: str) -> str:
            ...
            """
            failure_keywords = [
                f"{query} failure discontinued abandoned",                    # English
                f"{query} challenges problems obstacles",                     # English
                f"{query} Herausforderungen Probleme gescheitert abgebrochen", # German
                f"{query} Nachhaltigkeit Risiken Schwierigkeiten",            # German
                f"{query} défis problèmes échec abandonné",                   # French
                f"{query} durabilité risques difficultés",                    # French
            ]
    ...
    ("system", self.SYSTEM_PROMPT),
        ...
        return AgentExecutor(
            ...
            max_iterations=8, # Beyond 5 iterations, gains were negligible despite significant increases in token consumption
            return_intermediate_steps=True, # Keep True to see tool calls
            early_stopping_method="generate" # Forces a final answer when limit is hit
        )
